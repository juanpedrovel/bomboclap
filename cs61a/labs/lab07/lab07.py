...
def link_to_list(link):
    ...
    while pointer != Link.empty:
        regular_list.append(pointer.first)
...
def store_digits(n):
    ...
    n_string = str(n)
    link = False
    for digit in reversed(n_string):
        if not link:
            link = Link(int(digit))
        else:
...
def is_bst(t):
    ...
    """
    def bst_min(tree):
        if tree.is_leaf() or tree.branches[0].label > tree.label:
            return tree.label
        else:
            return bst_min(tree.branches[0])

    def bst_max(tree):
        if tree.is_leaf() or tree.branches[0].label < tree.label:
            return tree.label
        elif len(tree.branches) > 1:
            return bst_max(tree.branches[1])
        else:
            return bst_max(tree.branches[0])
