...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    list_closest_centroids = []
    for restaurant in restaurants:
        location = restaurant_location(restaurant)
        closest_centroid = find_closest(location, centroids)
        list_closest_centroids.append([closest_centroid, restaurant])
    return group_by_first(list_closest_centroids)
    # END Question 4
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    mean_latitude = mean([restaurant_location(r)[0] for r in cluster])
    mean_longitude = mean([restaurant_location(r)[1] for r in cluster])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    mean_xs = mean(xs)
    mean_ys = mean(ys)
    s_xx = sum([(xi - mean_xs) ** 2 for xi in xs])
    s_yy = sum([(yi - mean_ys) ** 2 for yi in ys])
    s_xy = sum([(xs[i] - mean_xs) * (ys[i] - mean_ys) for i in range(len(restaurants))])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            restaurants_ratings[rest_name] = user_rating(user, rest_name)
        else:
            restaurants_ratings[rest_name] = predictor(restaurant)
    return restaurants_ratings
    # END Question 9
