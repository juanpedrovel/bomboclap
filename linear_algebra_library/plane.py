...
class Plane:
    ...
    def plus(self, plane2):
        new_vector = self.normal_vector.plus(plane2.normal_vector)
        new_constant_term = self.constant_term + plane2.constant_term
        return Plane(new_vector, new_constant_term)
    ...
    def __eq__(self, plane2):
        if self.normal_vector.is_zero():
            if plane2.normal_vector.is_zero():
                difference = self.constant_term - plane2.constant_term
        ...
        if self.is_parallel(plane2):
            #connect a point of each plane and check if resulting vector is
            #orthogonal to the normal vectors of the planes
            vector_p1_to_p2 = plane2.basepoint.minus(self.basepoint)
